...
cp settings.env settings-web.env omero-centos6py27.env step04_all_omero.sh setup_omero_db.sh ~omero

su - omero -c "OMEROVER=$OMEROVER PY_ENV=py27_scl ICEVER=$ICEVER bash -eux step04_all_omero.sh"

su - omero -c "bash setup_omero_db.sh"

OMEROVER=$OMEROVER ICEVER=$ICEVER bash -eux step05_centos6_py27_nginx.sh
